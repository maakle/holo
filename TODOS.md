...
| Connections-page OAuth E2E | ✅ covered | `tests/e2e/tests/connections.spec.ts` is now active. Auth gate is exercised with a real signed Better Auth session via `tests/e2e/helpers/auth.ts` (drives `email-otp/send-verification-otp` → reads OTP from the `verification` table → `sign-in/email-otp`). The full GitHub roundtrip is intentionally not asserted: GitHub now uses an org-scoped App *installation* (not per-user OAuth), so mocking the install flow end-to-end is a separate item. |
