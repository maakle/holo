...
status: ACTIVE
---
# CEO Plan: memex v0.1 — YC-pitch-ready expansion
...
### 10x Check
The 10x version of memex doesn't ship more connectors. It ships **network effects on procedural knowledge**. Every team that adopts memex contributes anonymized skills (how their team handles X) into a shared registry. The first 100 published skills become the marketing growth loop — "How do top engineering teams handle GDPR escalations?" becomes a queryable corpus that brings the next 1000 teams in.

Beyond the marketplace: **memex graduates from middleware to operating system.** The "OS" framing isn't aspirational copy — it's a UI surface (agent observability dashboard + replay) that turns memex from "a thing my agent calls" into "a place my team goes when an agent acts weird." That's the difference between Stripe (infra) and Vercel (platform); same code, different shape.
...
- **CP4 — Agent templates:** productize the 3 v0.0 agents (support-question, interview-prep, customer-success) as YAML templates in `packages/agent-templates/`. Build once 3+ external CTOs are using memex and ask. Reasoning: keeps v0.1 scope tight; templates are most valuable when there's external demand to reuse them.
