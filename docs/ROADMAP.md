...
- [x] **CP3 — `npx holo init`.** macOS/Linux quickstart, GitHub-only at install (DX D44). Interactive prompts for the three quickstart keys (Anthropic + GitHub OAuth client id/secret), bundled docker-compose template that pulls pre-built images from GHCR. **Opt-in TTHW telemetry was removed by founder decision** — holo does not collect install or usage analytics; honest install-time numbers will be measured by other means (manual timing during dogfooding).
