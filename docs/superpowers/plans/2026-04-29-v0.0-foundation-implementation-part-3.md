# holo v0.0 Foundation — Implementation Plan, Part 3: Auth + Connectors
...
describe('validateSessionCookie', () => {
  ...
  it('throws HOLO_AUTH_NO_SESSION when no cookie header', async () => {
  ...
  it('throws HOLO_AUTH_NO_SESSION when token expired', async () => {
    await db.insert(schema.session).values({
      userId,
      token: 'expired-token',
...
- [ ] **Step 2: Repeat for `packages/skills`** with the same shape, name `@holo/skills`, placeholder export `'skills'`.

- [ ] **Step 3: Repeat for `packages/plans`** with the same shape, name `@holo/plans`, placeholder export `'plans'`.

- [ ] **Step 4: Verify all three install cleanly**

```bash
pnpm install
pnpm -F @holo/retrieval-core typecheck
pnpm -F @holo/skills typecheck
pnpm -F @holo/plans typecheck
```
