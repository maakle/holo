# memex v0.0 Foundation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the deployable skeleton for memex v0.0: monorepo, four apps, full day-1 schema with all indexes, Better Auth single-user (GitHub login + email OTP), encrypted-token storage, and a GitHub Connector OAuth handshake that roundtrips end-to-end. No ingestion, no MCP tools, no retrieval.
...
```json
{
  ...
  "scripts": {
    ...
    "test": "turbo run test",
    "db:migrate": "pnpm -F @memex/db migrate",
    "db:generate": "pnpm -F @memex/db generate"
  },
  "devDependencies": {
    "turbo": "2.1.3",
    "typescript": "5.6.3",
...
services:
  postgres:
    ...
    ports:
      - "5432:5432"
    volumes:
      - memex_pg_data:/var/lib/postgresql/data
    healthcheck:
...
```bash
docker compose exec postgres psql -U memex -d memex -c "CREATE EXTENSION IF NOT EXISTS vector; SELECT extversion FROM pg_extension WHERE extname='vector';"
```
...
### Task 1.1: `packages/errors` — MemexError class
...
- Create: `packages/errors/test/memex-error.test.ts`
...
- [ ] **Step 4: Write failing test `packages/errors/test/memex-error.test.ts`**
...
git commit -m "feat(errors): MemexError class with code/problem/cause/fix/docs_url"
```
...
git commit -m "feat(env): zod env schema with MEMEX_ENV_INVALID on parse failure"
```
