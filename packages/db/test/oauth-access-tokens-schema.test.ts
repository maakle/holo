...
describe('oauth_access_tokens schema', () => {
  ...
  it('has the unique index on token_hash', async () => {
    const rows = await sql<{ indexname: string; indexdef: string }[]>`
      SELECT indexname, indexdef FROM pg_indexes
       WHERE schemaname = 'public' AND tablename = 'oauth_access_tokens'
         AND indexname = 'oauth_access_tokens_token_hash_uniq'
    `;
