...
Step 2: Use `bash` (e.g. `cat /pylon/tickets/[ticket id].md`) on each P0/P1 ticket to summarize the incident, resolution time, and customer impact.
Step 3: Use `search` to find Grain call recordings with this customer (query: "[customer name] call").
Step 4: Use `bash` (e.g. `cat /grain/[date]/[title]-[id].md`) on the most recent 2–3 calls to extract key themes: feature requests, complaints, praise.
