...
Step 1: Use `bash` (e.g. `cat /pylon/tickets/[ticket id].md`) to read the full bug report including reproduction steps and affected account details.
Step 2: Use `search` to check for duplicate reports (query: "[error message or feature name] bug OR broken OR not working").
Step 3: Use `bash` (e.g. `cat /slack/#[channel]/[date]/thread-[ts].md`) on the customer's most recent Slack support thread to gather any additional context shared there.
