  ...
  try {
    ...
    if (!body?.token?.trim()) {
      throw holoError({
        ...
        fix: 'Paste your Linear personal API key (lin_api_…).',
      });
    }
    const token = body.token.trim();

    // Validate the token via the spec's testConnection — issues a viewer
    // query with the Authorization header injected by the apiKey auth.
    ...
    await db
      ...
      .onConflictDoUpdate({
        ...
        set: { name: ident.name, metadata: { linear_singleton: true }, updatedAt: new Date() },
      });
