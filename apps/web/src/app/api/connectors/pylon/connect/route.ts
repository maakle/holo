...
import { createPylonSpec } from '@holo/connectors';
  ...
  try {
    ...
    if (!body?.token?.trim()) {
      throw holoError({
        ...
        fix: 'Paste your Pylon API key.',
      });
    }
    const token = body.token.trim();

    // Validate key via the framework spec's testConnection. Build a one-shot
    // HTTP client with the user-supplied token so the spec doesn't need to
    // touch DB at this point.
    ...
    const existing = await db
      ...
      .where(
        and(
          ...
          eq(schema.connectorCredentials.provider, 'pylon'),
        ),
      );
    if (existing[0]) {
      await db
    ...
    await enqueueInitialSync(db, orgId, 'pylon').catch(() => {});
    ...
    emitAuditEvent({
      ...
      resourceId: 'pylon',
      meta: { provider: 'pylon', externalId: ident.externalId, name: ident.name },
    });
    ...
    if (e instanceof HoloError) {
      const status =
        e.code === 'HOLO_AUTH_NO_SESSION'
          ? 401
          : e.code === 'HOLO_PYLON_KEY_INVALID' || e.code === 'HOLO_ENV_INVALID' || e.code === 'HOLO_INVALID_INPUT'
            ? 400
            : 500;
