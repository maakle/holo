...
/**
 * Proxies the gateway's `GET /teams-bot/healthz` so the dashboard can
 * show "Verify deployment" results without dealing with CORS. The
 * dashboard sees gateway-truth (env vars on the gateway, not the web app)
 * because the gateway is the host that actually mints outbound Bot
 * Connector tokens.
 *
 * Session-gated, but unscoped to org — this is operator-facing data
 * about a shared deployment. Any signed-in user can see whether the
 * deployment is healthy. The bot secret is never returned; only the
 * mint-succeeded/failed signal.
 */
export async function GET() {
  try {
    ...
    const target = `${gatewayBase}/teams-bot/healthz`;
